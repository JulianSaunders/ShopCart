...
object ShopCart {
  ...
  sealed trait Offer {
    def calcPriceWithOffer(quantity: Int, item: Item): Int
  }

  class NoOffer extends Offer {
    override def calcPriceWithOffer(quantity: Int, item: Item): Int = quantity
  }

  class BuyOneGetOneFree extends Offer {
    override def calcPriceWithOffer(quantity: Int, item: Item): Int = {
  ...
  private def offerCheapestFree(m: Map[String, Int], p1: String, p2: String, products: Map[String, Item]): Map[String, Int] = {
    if (m.isDefinedAt(p1) && m.isDefinedAt(p2) && products(p1).price != products(p2).price) {
      ...
      m.map { (k, v) =>
        if (k.equals(cheapest)) {
          println(s"Offer CheapestFree applied for $k quantity=$v ,  freeUnits=$freeUnits ,  new quantity = ${v - freeUnits}  p1=$p1 p1Price=${products(p1).price} | p2=$p2 p2Price=${products(p2).price} | cheapest=$cheapest")
          (k, v - freeUnits)
        } else (k, v)
      }
    } else m
  ...
  def calcTotal(items: List[String], products: Map[String, Item]): BigDecimal = {
    ...
    val itemCountsAfterOffers1 = itemCounts.map((k, v) => (k, products(k).offer.calcPriceWithOffer(v, products(k))))
    println(s"itemCountsAfterOffers1= $itemCountsAfterOffers1\n-------")

    val itemCountsAfterOffers2 = offerCheapestFree(itemCountsAfterOffers1, "apple", "banana", products)
